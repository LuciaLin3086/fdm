...
class ExplicitFiniteDifference:
    ...
    def get_coefficient_matrix(self):
        ...
        for j in range(self.m + 1):
            a_ls[j] = (- (self.r - self.q)/2 * j * self.delta_T + self.sigma ** 2 * j ** 2 * self.delta_T / 2) / (1 + self.r * self.delta_T)
            b_ls[j] = (1 - self.sigma ** 2 * j ** 2 * self.delta_T)/ (1 + self.r * self.delta_T)
            c_ls[j] = ((self.r - self.q)/2 * j * self.delta_T + self.sigma ** 2 * j ** 2 * self.delta_T / 2) / (1 + self.r * self.delta_T)


        coeff_matrix = np.zeros((self.m - 1, self.m + 1))
        for j in range(1, self.m):  # j = 1,2,...,m-2,m-1
            for i in range(j - 1, j + 2):  # i = j-1, j, j+1
                # 注意計算a,b,c時的j是如講義中由下往上數的
                # coeff_matrix的index是從上往下數的，所以 m - (j - 1) - 1
                if i == j - 1: # diagonal element
                    coeff_matrix[j - 1, i] = c_ls[self.m - (j - 1) - 1] # j-1 = 0,1,2,...,m-3,m-2
                elif i == j:
                    coeff_matrix[j - 1, i] = b_ls[self.m - (j - 1) - 1]
                elif i == j + 1:
                    coeff_matrix[j - 1, i] = a_ls[self.m - (j - 1) - 1]
    ...
    def backward_induction(self):
        ...
        for i in range(self.n - 1, -1, -1):  # i = n-1,n-2,...,2,1,0
            ...
            if self.E_A == "A": # American
                ...
                for j in range(1, self.m):  # j = 1,2,...,m-1
                    # 注意unknown_f_ls的index是從0開始
                    f_matrix[j, i] = unknown_f_ls[j - 1] # j - 1 = 0,1,...,m-2
                    # To minimize the pricing error, f_matrix[j, i] must be positive.
                    f_matrix[j, i] = max(f_matrix[j, i], 0)
